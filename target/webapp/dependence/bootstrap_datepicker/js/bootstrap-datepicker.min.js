(function (a) {
    ...
    var g = function (k, j) {
        ...
        this.viewSelect = (this.minView === 0)? -1:this.minView;
        if ("viewSelect" in j) {
        ...
        this.picker = d(l).appendTo(this.isInline ? this.element : this.container).on({
            ...
            change: d.proxy(this.change, this),
        ...
        if (this.isInline) {
            ...
            this.picker.addClass("datetimepicker-dropdown-" + this.pickerPosition + " datepicker-dropdown")
        }
        if (this.isRTL) {
    ...
    g.prototype = {
        ...
        fill: function () {
            ...
            //添加秒视图
            var addSec = /,s{1,2},/.test(',' + this.format.parts.join(',') + ','), sSec = addSec ? '<select style="width:100%;outline:none" class="custom-date-select">' : '';
            this.addSec = addSec;
            if (addSec) {
                var orgSec = this.viewDate.getSeconds();//默认则改成0
                for (var _i = 0; _i < 60; _i++) sSec += '<option value="' + _i + '"' + (_i == orgSec ? ' selected' : '') + '>' + (_i < 10 ? '0' : '') + _i + '</option>';
                sSec += '</select>';
            }
            // d(sSec).change(function (e) {
            //     console.log('asdasdhsj')
            // })
            this.picker.find(".datetimepicker-minutes td").html(r.join("")+sSec);
            var that = this;
            this.picker.find(".datetimepicker-minutes td .custom-date-select").change(function (e) {
        ...
        change:function (m) {
            ...
            m.preventDefault();
            var d = this.dateTemp;
            this._setDate(h(d[0],d[1],d[2],d[3],d[4],$(m.target).val(),0));
            this.fill();
            if (this.autoclose) {
                this.hide()
            }
        ...
        click: function (m) {
            m.stopPropagation();
            m.preventDefault();
            if (m.target.tagName == 'SELECT' || m.target.tagName == 'OPTION') return;
                    ...
                    case"span":
                        if (!n.is(".disabled")) {
                            var p = this.viewDate.getUTCFullYear(), o = this.viewDate.getUTCMonth(),
                                ...
                                k = this.viewDate.getUTCMinutes(),// this.minView ===1 ? 0 : this.viewDate.getUTCMinutes()
                                s = this.addSec ? this.picker.find('select').val() : this.viewDate.getUTCSeconds() ;//修改datetimepicker秒钟获取方式
                            if (n.is(".month")) {
    ...
    var c = {
        ...
        parseDate: function (A, y, v, j, r) {
            ...
            if (/^\d{4}\d{1,2}\d{1,2}$/.test(A) || /^\d{4}\d{1,2}$/.test(A)) {
                q = [];
                var locationId = 0;
                for (var i = 0, j = y.parts.length; i < j; i++) {
                    q.push(A.toString().substr(locationId, y.parts[i].length));
                    locationId += y.parts[i].length;
                }
            }
            var A = new Date(0, 0, 0, 0, 0, 0, 0), m = {},
    ...
    !function($) {
        ...
        Dropdown.prototype = {
            ...
            keydown: function(e) {
                ...
                if (/(38|40|27)/.test(e.keyCode) && ($this = $(this),
                        e.preventDefault(),
                        e.stopPropagation(),
                    ...
                    $items.length && (index = $items.index($items.filter(":focus")),
        ...
        $.fn.dropdown = function(option) {
            ...
            $(document).on("click.dropdown.data-api touchstart.dropdown.data-api", clearMenus).on("click.dropdown touchstart.dropdown.data-api", ".dropdown form", function(e) {
                e.stopPropagation()
            }).on("touchstart.dropdown.data-api", ".datepicker-dropdown", function(e) {
                e.stopPropagation()
